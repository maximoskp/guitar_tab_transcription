...
for i in session_ids:
    for j in range(frames_per_session):
        ...
        fig.savefig( '../figs/tab_flat_CNN_out/session_'+str(i)+'/fig_'+str(j)+'.png', dpi=300 )

# %% training curves

import pandas as pd

df = pd.read_csv('../models/tab_flat_CNN_out/flat_tab_logger.csv', delimiter=';')
...
fig1.savefig('../figs/training_curves/' + 'losses.pdf')
fig2 = cossims.plot().get_figure()
fig2.savefig('../figs/training_curves/' + 'cossims.pdf')

original_stdout = sys.stdout
with open('../figs/training_curves/best_epoch.txt', 'w') as f:
    sys.stdout = f
